    ...
    return (
      <div className="min-h-screen flex items-center justify-center" dir={dir}>
        <div className="text-center">
          ...
          <Link to="/shop"><Button className="rounded-full bg-violet-600">{t.shop_now}</Button></Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50" dir={dir}>
      <nav className="bg-white border-b border-slate-200 px-4 sm:px-6 h-14 flex items-center gap-3 sticky top-0 z-30">
        ...
        <h1 className="font-bold">{t.checkout_title}</h1>
      </nav>

      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        {/* Steps */}
        <div className="flex items-center gap-2 justify-center">
          {[{n:1,l:t.shipping_info},{n:2,l:t.payment_method},{n:3,l:t.order_summary}].map((s,i) => (
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-2xl border border-slate-100 p-6 space-y-4">
            <h2 className="font-bold text-lg">{t.shipping_info}</h2>
            <div className="grid sm:grid-cols-2 gap-3">
              <div><label className="text-sm font-medium">{t.full_name} *</label><Input className="mt-1 rounded-xl" value={form.customer_name} onChange={e => set('customer_name', e.target.value)} /></div>
              <div><label className="text-sm font-medium">{t.phone} *</label><Input className="mt-1 rounded-xl" value={form.customer_phone} onChange={e => set('customer_phone', e.target.value)} /></div>
              <div><label className="text-sm font-medium">{t.email}</label><Input className="mt-1 rounded-xl" type="email" value={form.customer_email} onChange={e => set('customer_email', e.target.value)} /></div>
              <div><label className="text-sm font-medium">{t.city} *</label><Input className="mt-1 rounded-xl" value={form.shipping_city} onChange={e => set('shipping_city', e.target.value)} /></div>
              <div className="sm:col-span-2"><label className="text-sm font-medium">{t.address} *</label><Input className="mt-1 rounded-xl" value={form.shipping_address} onChange={e => set('shipping_address', e.target.value)} /></div>
            </div>
            <Button onClick={() => setStep(2)} className="w-full rounded-full bg-violet-600 font-bold">{t.save}</Button>
          </motion.div>
        )}
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-2xl border border-slate-100 p-6 space-y-4">
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setStep(1)} className="flex-1 rounded-full">{t.cancel}</Button>
              <Button
                onClick={() => {
        ...
        {step === 3 && (
          <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-3">
              <h2 className="font-bold">{t.order_summary}</h2>
              {items.map(item => (
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setStep(2)} className="flex-1 rounded-full">{t.cancel}</Button>
              <Button onClick={placeOrder} disabled={loading} className="flex-1 rounded-full bg-violet-600 font-bold gap-2">
                <CheckCircle className="w-4 h-4" /> {loading ? t.loading : t.place_order}
              </Button>
            </div>
          </motion.div>
        )}
